...
class FaceDetector:
    ...
    def __init__(self, faceLandmarker, fps, marThresh, marThresh2, headThresh, earThresh, blinkThresh, cameraMatrix=None, distCoeffs=None):
    ...
    def detect_eyes(self, frame, results, roll, pitch, yaw, display):
        ...
        # New API: results.face_landmarks is a list of face landmark lists
        if results.face_landmarks:
            landmarks = results.face_landmarks[0]

            # Convert normalized landmarks to pixel coordinates
            meshCoords = [(int(lm.x * self.imgW), int(lm.y * self.imgH)) for lm in landmarks]
            ...
            # No face detected — set safe defaults
            self.sleepEyes = False
            self.baseR = self.baseP = self.baseY = self.baseG = 0
